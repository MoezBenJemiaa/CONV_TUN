...
const FilterSidebar = ({ filters, setFilters, rides }) => {
  ...
  rides.forEach((ride) => {
    ...
    if (ride.preferences.includes("maxTwoBack"))
      serviceCounts["Max 2 à l’arrière"]++;
    if (ride.reservationType === "instant")
      serviceCounts["Réservation instantanée"]++;
    if (ride.preferences.includes("smokingAllowed"))
      serviceCounts["Cigarette autorisée"]++;
    if (ride.preferences.includes("heavyLuggage"))
      serviceCounts["Bagage lourd"]++;
  });
  ...
  return (
    <div className="filter-sidebar">
      <div className="filter-section">
        ...
        {["departEarliest", "lowestPrice", "shortestTrip"].map(
          (value, index) => (
            <label key={value}>
              <input
                type="radio"
                ...
                onChange={() => setFilters({ ...filters, sortBy: value })}
              />
              {
                [
                  "Départ le plus tôt",
      ...
      {["departureTimes", "services"].map((category, index) => (
        <div className="filter-section" key={category}>
          <div className="filter-title">
            {["Heure de départ", "Services"][index]}
          </div>
          {Object.entries(index === 0 ? departureCounts : serviceCounts).map(
            ([name, count]) => (
              <label key={name}>
                <input
                  type="checkbox"
