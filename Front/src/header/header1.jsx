  ...
  useEffect(() => {
    const fetchUnseen = async () => {
      ...
      try {
        ...
        const res = await axios.get(`http://localhost:5000/notifications/unseen/${id}`);
        setHasUnseen(res.data.unseenCount > 0);
      } catch (err) {
  ...
  return (
    <nav className={styles.navbar}>
      ...
      <div className={styles.userSection} ref={dropdownRef}>
        ...
        <div className={`${styles.profile_container} ${styles.relative}`} ref={dropdownRef}>
          {hasUnseen && <span className={styles.redDot} />}
          {profilePic ? (
            <img
              src={profilePic}
              alt="Profile"
                ...
                <ul>
                  {token ? (
                    <>
                      ...
                      <li className={`${styles.dropdownItem} ${styles.relative}`} onClick={goToNotifications}>
                        Notifications
                        {hasUnseen && <span className={styles.redDot} />}
                        <ChevronRight className={styles.chevron} />
                      </li>
