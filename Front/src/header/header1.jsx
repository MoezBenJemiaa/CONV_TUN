...
function Navbar() {
  ...
  const GoToReservations = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/reservation/user/${userID}`);
  ...
  return (
    <nav className={styles.navbar}>
      ...
      <div className={styles.userSection} ref={dropdownRef}>
        ...
        <button className={styles.plusIcon} onClick={handleNavigate}>
          <FontAwesomeIcon icon={faPlus} size="lg" />
        </button>
        <p className={styles.parag} onClick={handleNavigate}>Publier un trajet</p>               
            ...
            <div className={styles.profile_container} ref={dropdownRef}>
                        {profilePic   ? (
                          ...
                          />
                            ...
                            <FontAwesomeIcon icon={faCircleUser} size="2x" onClick={() => setDropdownOpen(!dropdownOpen)} />
                        )}
            </div>
            ...
            {(() => {  
              ...
              return (
                <ul>
                  {token ? (
                    <>
                      <li className={styles.dropdownItem} onClick={GoToProfile}>Profile</li>
                      <li className={styles.dropdownItem} onClick={handMesTrajets}>Mes trajets</li>
                      <li className={styles.dropdownItem} onClick={GoToReservations}>Reservations</li>
                      <li className={`${styles.dropdownItem} ${styles.logout}`}>Notifications</li> 
                      <li className={`${styles.dropdownItem} ${styles.logout} ${styles.logoutt}`} onClick={handleLogout}>
                        <button>LogOut</button>
                      </li>
                    </>
                  ) : (
                    <>
                      <li className={styles.dropdownItem} onClick={GoToLogin}>Connectez-vous</li>
                      <li className={styles.dropdownItem} onClick={GoToRegister}>Inscrivez-vous</li>
                    </>
                  )}
                </ul>
              );
